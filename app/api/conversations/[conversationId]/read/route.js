...
export async function POST(request, { params }) {
    try {
        ...
        console.log(`[READ] User ${session.user.id} marking conv ${conversationId} as read`);
        ...
        // DIAGNOSTIC DUMP
        try {
            const dumpMsgs = await db.collection('messages').find({ conversationId }).limit(3).toArray();
            ...
            dumpMsgs.forEach((msg, i) => {
                ...
                const isMeInList = msg.readBy?.some(rb => rb.userId == session.user.id);

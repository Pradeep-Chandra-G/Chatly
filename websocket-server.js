...
io.on("connection", (socket) => {
  ...
  socket.on("message:send", (data) => {
    ...
    const conversationSockets = io.sockets.adapter.rooms.get(data.conversationId);
  ...
  socket.on("message:edit", ({ messageId, conversationId, content, edited, editedAt }) => {
    io.to(conversationId).emit("message:edited", { messageId, content, edited, editedAt });
  });

  socket.on("call:initiate", ({ callId, receiverId, type, offer }) => {
    const receiverSocket = userSockets.get(receiverId);
    if (receiverSocket) {
      io.to(receiverSocket).emit("call:incoming", { callId, callerId: socket.userId, type, offer });
    }
  });
  ...
  socket.on("call:ice-candidate", ({ targetId, candidate }) => {
    ...
    if (targetSocket) {
      io.to(targetSocket).emit("call:ice-candidate", { senderId: socket.userId, candidate });
    }
  });
  ...
  socket.on("message:reaction", ({ messageId, conversationId, reactions }) => {
    io.to(conversationId).emit("message:reaction-update", { messageId, reactions });
  });
